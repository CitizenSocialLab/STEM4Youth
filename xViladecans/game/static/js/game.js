    ...
    end_round:function(data) {
        ...
        for(i=0; i<6; i++)
        {
            if (this.id_user == this.ids[i]) {
                $('#player_'+(i+1)).css("font-weight", "bold").css("color", "#035992");
                $('#resultat_seleccio_'+(i+1)).css("font-weight", "bold").css("color", "#035992");
                $('#resultat_actual_'+(i+1)).css("font-weight", "bold").css("color", "#035992");
                $('#resultat_inicial_'+(i+1)).css("font-weight", "bold").css("color", "#035992");
            }
    ...
    $("#final-modal").on("shown.bs.modal", function() {    // wire up the OK button to dismiss the modal when shown
        $("#final-modal-fi").on("click", function(e) {
            $("#final-modal").modal('hide').on("hidden.bs.modal", function() {
                window.location.href = '/'+ game.lang + '/user/enquestafinalintro';
            });
        });
    });
